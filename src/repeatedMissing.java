...
//
//        return D;
        long diff =A.get(0)-A.size();

        long sqdiff = (long)(Math.pow(A.get(0),2)-Math.pow(A.size(),2));
        ArrayList<Integer>C = new ArrayList<Integer>();

        for(int i =1; i<A.size();i++){
        diff = diff + A.get(i)-i;
        sqdiff = sqdiff + (long)(Math.pow(A.get(i),2)-Math.pow(i,2));
        }
        int sum = (int) (sqdiff/diff);

        System.out.println("diff is "+diff+" and sqdiff is "+sqdiff+" sum is "+sum);
